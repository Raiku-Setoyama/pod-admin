...
**アプリの正本は別リポジトリ（`illustrator-vm`）である。** ここに書くのは
**移送によって変わった部分**と、**pod-admin 側で先に止めるもの**だけで、
Windows 側の手順の正本は `illustrator-vm/scripts/setup_windows/README.md` である。

#### 先に pod-admin のワーカーを止める

**これを忘れると、待ち行列が全滅する。**

```bash
gcloud scheduler jobs pause pod-admin-worker --location=asia-northeast1 --project=tosyo-api-504104
#   … VM 側の更新 …
gcloud scheduler jobs resume pod-admin-worker --location=asia-northeast1 --project=tosyo-api-504104
```

VM が応答しない間にワーカーが動くと、`generate()` の例外ハンドラが行を
**`failed` に確定させる**（`api/app/services/manufacturing_data_service.py`）。
**`failed` は終端でリトライが無い** — `reclaim_expired_leases()` が拾うのは
リースの切れた `generating` だけである。しかもワーカーは待ち行列が空になるまで回るので
（`api/app/worker.py`）、**1 回の起動で溜まっていた行がまとめて落ちる。**

止めておけば行は `pending` のまま待ち、VM が戻った瞬間に流れる。
REQ-0055 の移送では、この手順で 9 分 30 秒止めて `failed` を 1 件も増やさなかった。

止める前に、処理中のジョブが無いことを確かめる。**VM は 1 件ずつの直列処理である。**
...
curl -s localhost:18000/health   # queue.current_job_id が null、active_jobs が 0
```
...
サーバーは**タスクスケジューラのタスク `IllustratorAPIServer`** が動かしている。

```powershell
cd C:\illustrator-vm
git pull

Stop-ScheduledTask  -TaskName IllustratorAPIServer
Get-Process python -EA SilentlyContinue | Stop-Process -Force
Start-ScheduledTask -TaskName IllustratorAPIServer

Invoke-RestMethod http://localhost:8000/health
Get-Content C:\illustrator-vm\logs\server.log -Tail 20
```

**外してはいけない点が 3 つある**（理由は向こうの README にある）。

- **`python run_server.py` と `start_server.ps1` を本番で使わない。** コンソールに紐づくため、
  RDP を切断すると uvicorn が出力でブロックして応答停止する
- **`Stop` → python 停止 → `Start` の順を守る。** 単一インスタンスガードがあるので、
  古いプロセスが残っていると新版が起動しない
- **切断はデスクトップの「Disconnect RDP (Keep Session)」ショートカットで行う。**
  × で閉じるとデスクトップがロックされ、**GUI を必要とする Illustrator が止まる**

#### 更新後の確認

**起動しただけでは、Illustrator が描けるかは分からない。** 実際に 1 件生成を通す。

```bash
curl -s localhost:18000/          # version が上がったか
curl -s localhost:18000/health    # config_loaded / worker_running が true か
```

そのうえで生成まで確かめる方法は「製造データ生成の疎通確認」の節と同じである。
**トンネル越しに VM へ直接投げれば、pod-admin の DB にも GCS にも触れずに検証できる**
（REQ-0055 の段階 3 で使った方法）。

...
**事前チェック（いずれかが失敗したらFAIL）:**

1. **テストファイルの存在確認**:
```bash
# e2e/ディレクトリとテストファイルの存在を確認
if [ ! -d "e2e" ] || [ -z "$(find e2e -name '*.spec.ts' -o -name '*.spec.js' 2>/dev/null)" ]; then
...
## 判定
- PASS: Layer 1-4 全パス + Layer 5-6 重大問題なし
- 要確認: Layer 1-4 パスだが懸念あり
- FAIL: Layer 1-4 のいずれか失敗

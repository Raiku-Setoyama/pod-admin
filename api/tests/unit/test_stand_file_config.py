...
class TestGetStandFilePath:
    ...
    def test_fallback_to_default_for_unknown_size(self) -> None:
...
class TestLoadStandFile:
    ...
    def test_returns_content_when_file_exists(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """ファイルが存在する場合に内容を返す"""
    ...
    def test_returns_none_when_file_not_found(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_correct_file_for_size(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestStandFileMapping:
    ...
    def test_all_filenames_end_with_ai(self) -> None:
        """全ファイル名が .ai で終わること"""
        for value in STAND_FILE_MAPPING.values():

...
class TestIsBusinessDay:
    ...
    def test_japanese_holiday_is_not_business_day(self):
    ...
    def test_non_company_holiday_is_business_day(self):
        """独自休日以外の平日は営業日."""
...
class TestAddBusinessDays:
    ...
    def test_negative_days_returns_start_date(self):
        """負の営業日はstart_dateをそのまま返す."""
        result = add_business_days(date(2026, 4, 9), -1)
        assert result == date(2026, 4, 9)

    def test_simple_weekday_addition(self):
        """平日のみの加算（週末をまたがない）."""
        # 月曜日から3営業日 → 木曜日
    ...
    def test_skip_company_holiday(self):
        """独自休日をスキップ."""
    ...
    def test_company_holidays_with_weekend(self):
        """独自休日と週末が重なるケース."""
    ...
    def test_consecutive_company_holidays(self):
        """連続する独自休日（夏季休暇など）."""
        # 8/11(火)〜8/14(金)が夏季休暇
        holidays = {
            date(2026, 8, 11),
    ...
    def test_start_date_itself_not_counted(self):
